...
The distribution identity is intentionally separate from the upstream app: `T3 Code TJN`, `dev.neuman.t3code`, custom user-data names, and `t3code-tjn` protocol names. Packaged updates use the fork-backed GitHub Releases feed `travisjneuman/t3code`; Nightly builds select the `nightly` prerelease channel. The updater must only install packages built with this same identity. The fork's `TJN Sync Upstream Source` workflow polls `pingdotgg/t3code` every 15 minutes and performs a non-destructive merge into `main`; conflicts fail before push. The existing Release workflow publishes the merged source as the fork's Nightly package, so unpublished upstream branch changes are not silently treated as a release baseline.
