...
Use this when you want to connect to a ndev.t3code server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch ndev.t3code over SSH.
...
Use this when you want the desktop app to start or reuse ndev.t3code on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. ndev.t3code uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, ndev.t3code first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path ndev.t3code uses:
...
When the ndev.t3code web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: ndev.t3code may
...
see [Keeping ndev.t3code in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running ndev.t3code in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through ndev.t3code. The browser still connects directly to the backend URL in the pairing link.
