...
machine running your ndev.t3code server does that, its themes appear in the theme library alongside
your own, and ndev.t3code follows them: change the desktop theme and ndev.t3code retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. ndev.t3code Mobile keeps its own
...
the standard ndev.t3code look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the ndev.t3code state
...
exported from ndev.t3code (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. ndev.t3code generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a ndev.t3code theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of ndev.t3code does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so ndev.t3code never reads
