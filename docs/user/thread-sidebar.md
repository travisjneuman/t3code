...
If reordering is unavailable for one environment, update the ndev.t3code server running in that
...
pill** fallback because their colors are not controlled by ndev.t3code.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While ndev.t3code is generating it, the action reads **Regenerating…** and cannot
