# Keeping ndev.t3code in Sync

The ndev.t3code web or desktop app and the server it connects to work best when they use the same
version. If they do not match, ndev.t3code shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the ndev.t3code Linux background service. Select the button and leave ndev.t3code open while it prepares, tests, restarts, and reconnects.                        |
| **Update the desktop app** | Open the ndev.t3code desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current ndev.t3code server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. ndev.t3code does not update connected
...
If the server instead runs as the ndev.t3code background service, update the service on the host and
...
See [Running ndev.t3code in the Background](./background-service.md) for install, status, and removal
commands.
