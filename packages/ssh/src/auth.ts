...
# Invoked by ssh via SSH_ASKPASS when ndev.t3code re-runs ssh with a cached password
...
printf 'ndev.t3code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when ndev.t3code re-runs\r
...
[Console]::Error.WriteLine("ndev.t3code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
