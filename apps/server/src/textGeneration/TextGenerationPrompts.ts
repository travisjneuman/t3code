...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this ndev.t3code thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing ndev.t3code thread so the user can recognize it weeks later.
