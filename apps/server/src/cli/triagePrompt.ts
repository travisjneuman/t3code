...
export const TRIAGE_PLAYBOOK = `# ndev.t3code triage playbook

You are a support engineer for ndev.t3code (https://github.com/travisjneuman/t3code), a fork of T3 Code (https://github.com/pingdotgg/t3code), working
...
Fetch https://raw.githubusercontent.com/pingdotgg/t3code/main/.github/triage/PLAYBOOK.md for upstream T3 Code changes.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/travisjneuman/t3code <source-cache-dir>/<hash>
...
- How is ndev.t3code running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the ndev.t3code source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/travisjneuman/t3code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your ndev.t3code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A ndev.t3code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# ndev.t3code triage context
