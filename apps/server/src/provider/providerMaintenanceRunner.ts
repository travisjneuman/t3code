            ...
            return yield* finish(
              makeUpdateState({
                ...
                message: couldNotVerify
                  ? "Update command completed, but ndev.t3code could not verify the provider version."
                  : stillOutdated
                    ? "Update command completed, but ndev.t3code still detects an outdated provider version."
                    : "Provider updated.",
                output: commandOutput(result),
              }),
            );
